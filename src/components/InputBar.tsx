            ...
            {hasAnySelection ? (
              <div className="flex justify-center py-3">
                <div className="bg-gray-800/90 dark:bg-gray-800/90 backdrop-blur shadow-lg rounded-full flex items-center p-1 border border-white/10">
                  <button onClick={() => { clearSelection(); setSelectedTemplateIds([]) }} className="p-2 text-gray-300 hover:text-white transition-colors" title="取消选择">
                    ...
                    <>
                      ...
                      <button
                        onClick={() => {
                          setConfirmDialog({
                            title: '批量删除模板',
                            message: `确定要删除选中的 ${selectedTemplateIds.length} 个模板吗？`,
                            action: () => { void removeMultipleTemplates(selectedTemplateIds).then(() => setSelectedTemplateIds([])) },
                          })
                        }}
                        className="p-2 text-red-400 hover:text-red-300 transition-colors" title="删除选中模板"
                      >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                      </button>
                      <span className="px-2 text-xs text-gray-400">{selectedTemplateIds.length} 个模板</span>
                    </>
                  )}
                </div>
              </div>
